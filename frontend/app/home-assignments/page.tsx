...
export default function HomeAssignmentsPage() {
  const [data, setData] = useState<HomeAssignmentsPayload | null>(null);
  const [draft, setDraft] = useState<HomeAssignmentsPayload | null>(null);
  ...
  async function load() {
    ...
    try {
      const [homeRes, employeesRes] = await Promise.all([
        fetch("/api/home-assignments", { cache: "no-store" }),
        fetch("/api/employees", { cache: "no-store" }),
      ]);

      if (!homeRes.ok) throw new Error("Failed to load home assignments");

      const homeJson = (await homeRes.json()) as HomeAssignmentsPayload;
      ...
      for (const section of homeJson.sections) {
        for (const employee of section.employees) {
      ...
      setError(err instanceof Error ? err.message : "Failed to load home assignments");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/home-assignments", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Failed to save home assignments");
      }

      const payload = json.payload as HomeAssignmentsPayload;
      setData(payload);
      setDraft(payload);
      setMoveState(null);
      setSaveState("saved");
      ...
      setSaveState("error");
      setError(err instanceof Error ? err.message : "Failed to save home assignments");
    }
  }
      ...
      <main className="min-h-screen bg-slate-100">
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="text-sm text-slate-600">Loading home assignments...</div>
          </div>
        </div>
      </main>
    );
      ...
      <main className="min-h-screen bg-slate-100">
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          <div className="rounded-2xl border border-red-200 bg-white p-6 shadow-sm">
            <div className="text-sm text-red-700">{error || "Failed to load home assignments."}</div>
          </div>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-950">Home Assignments</h1>
              <p className="mt-2 text-sm text-slate-600">
                Type freely, click the closest available match, or use Move mode to reassign people
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {saveState === "saving" ? "Saving..." : "Save Homes"}
              </button>
            </div>
          </div>
